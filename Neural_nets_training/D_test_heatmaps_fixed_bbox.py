...
# Import the custom preprocessor from your dataloader file
...
checkpoint_path = r"C:\Users\CYBER ARTEL\PycharmProjects\atelectasis_classification_and_detection\Neural_nets_training\best_deit_scm_model_2.pth"

# --- Updated transforms to match training preprocessing ---
transform = transforms.Compose([
    DicomStyleImagePreprocessor(target_size=(224, 224), apply_clahe=True),  # Same as training
    transforms.ToTensor(),
...
def compute_giou(boxA, boxB):
    ...
    unionArea = boxAArea + boxBArea - (
        iou * unionArea if (unionArea := boxAArea + boxBArea - iou * boxAArea) != 0 else 0)
...
# --- Helper function to get preprocessed image for visualization ---
def get_preprocessed_image_for_visualization(original_image):
    """Get the preprocessed image as it would appear after DicomStyleImagePreprocessor"""
    preprocessor = DicomStyleImagePreprocessor(target_size=(224, 224), apply_clahe=True)
    processed_pil = preprocessor(original_image)
...
for filename in os.listdir(image_folder):
    ...
    # Get preprocessed image for visualization (matches what the model actually sees)
    img_vis = get_preprocessed_image_for_visualization(original_image)
    ...
    if gt_bbox_orig:
        ...
        if predicted_bbox:
            ...
            cv2.putText(overlayed, f"GIoU: {giou:.2f}", (10, 40),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
            cv2.putText(overlayed, f"Hausdorff: {hausdorff:.2f}", (10, 60),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
...
if avg_boundary_distances:
    print(
        f"Mean Average Boundary Distance over {len(avg_boundary_distances)} samples: {np.mean(avg_boundary_distances):.4f}%")