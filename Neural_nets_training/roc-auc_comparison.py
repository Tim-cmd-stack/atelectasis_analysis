...
def main():
    # Настройки
    data_dir = r"C:\Users\CYBER ARTEL\.cache\kagglehub\datasets\nih-chest-xrays\data\nih_custom_dataset\new_classes"
    ...
    # Преобразования (без аугментации для тестирования)
    ...
    full_dataset = EvaluationDataset(data_dir, transform=transform)
    ...
    # Создание DataLoader только для тестовой выборки
    ...
    print(f"\nИспользуется тестовая выборка: {len(test_dataset)} образцов")
    ...
    print(f"Стратификация: сохранен исходный баланс классов")
    ...
    print("\nДетальный отчет классификации (тестовая выборка):")
    ...
    print(f"Точность на тестовой выборке: {len(correct_predictions) / len(results_df) * 100:.2f}%")
    ...
    if len(uncertain_predictions) > 0:
        ...
        print(f"Точность среди неуверенных: {len(uncertain_predictions[uncertain_predictions['Correct_Prediction'] == True]) / len(uncertain_predictions) * 100:.1f}%")
